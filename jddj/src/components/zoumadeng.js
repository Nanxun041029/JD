...
<div id="zoumadeng">
	<div className="need-left-right-margin">
	    ...
	    >
	      {this.props.lbt.map((item,index)=>{
	      	return <div key={item+index} className="lb">
	...
	return {
		gotData:function(){
			...
			.then((res)=>{
				...
				dispatch({
					type:"ZMD_GET_DATA",
					payload:res.data.result.data[0].data
				})
			})
		}
...
const Zoumadeng = connect(mapStateToProps,mapDispatchToProps)(ZoumadengUI);
